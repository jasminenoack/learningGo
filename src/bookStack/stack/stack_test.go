...
func TestStack(t *testing.T) {
	...
	count++
	assertTrue(t, aStack.IsEmpty() == false, "expected nonempty stack", count) // 9
	count++

	value2, err := aStack.Pop()
	assertEqualString(t, value2.(string), "three", "unexpected text", count) // 10
	count++
	...
	count++
	assertTrue(t, aStack.IsEmpty() == false, "expected nonempty stack", count) // 15
	count++
